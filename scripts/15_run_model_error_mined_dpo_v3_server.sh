...
# DPO-only resume after local model-error pair mining. The 960 rollouts are not regenerated.
...
ARCHIVE_PATH="$ARCHIVE_DIR/model_error_mined_dpo_v3_$RUN_ID.tar.gz"
...
log "model_error_mined_dpo_v3_run_id=$RUN_ID"
nvidia-smi
(
  while true; do
    date -Is
...
python tools/make_dpo_v3_inference_config.py \
  --adapter "$SELECTED" \
  --predictions_dir "outputs/predictions/phase10_dpo_v3_selected_train_decode_dev" \
  --ground_truth_dir "outputs/eval_sets/phase10_dpo_v3_selected_train_decode_dev" \
  --output "$FINAL_CONFIG"
...
tar -czf "$ARCHIVE_PATH" \
  ...
  "outputs/eval_reports/phase10_dpo_v3_model_mined/dpo_v3_reward_audit.json" \
  "$DPO_CONFIG" "scripts/15_run_model_error_mined_dpo_v3_server.sh" \
