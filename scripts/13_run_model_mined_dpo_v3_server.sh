...
ARCHIVE_PATH="$ARCHIVE_DIR/model_mined_dpo_v3_$RUN_ID.tar.gz"
...
python -m py_compile   src/mv_audit/training/reward_function.py   src/mv_audit/training/train_dpo.py   src/mv_audit/training/train_sft.py   src/mv_audit/inference/batch_inference.py   src/mv_audit/inference/sample_model_mined.py   tools/model_mined_dpo_v3.py   tools/merge_inference_shards.py
python -m pytest -q tests/test_model_mined_dpo_v3.py tests/test_dpo_loss_types.py   tests/test_schema_guard.py tests/test_prompt_order_id_guard.py
python scripts/test_reward_function.py
DRY_RUN_ONLY=1 bash scripts/12_run_order_id_repair_sft_v3_server.sh
log "validation_done"

log "sft_v3_train_start"
ALLOW_TRAINING=1 bash scripts/12_run_order_id_repair_sft_v3_server.sh   > "$LOG_DIR/sft_v3_train.log" 2>&1
require_file "$R3_ADAPTER/adapter_config.json"
log "sft_v3_train_done"

run_sharded_inference "$SFT_CONFIG" repair_sft_r3 sft-v3
...
evaluate_predictions "$SFT_GT" "$SFT_PRED" "$RUN_ROOT/eval/sft_v3"
